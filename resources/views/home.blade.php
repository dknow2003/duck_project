					...
					<div class="row">
						...
						<div class="col-xs-4">
							...
							<div class="col-xs-8 text-left">
								...
								<h2 class="font-bold">{{ $data['registers'] }} / {{ $data['actives'] }} / {{ $data['payed'] }}</h2>
							</div>
						</div>
						...
						<div class="col-xs-4">
							...
							<div class="col-xs-8 text-left">
								<span>收入</span>
								<h2 class="font-bold">{{ $data['amount'] }}</h2>
							</div>
						</div>
						...
						<div class="col-xs-4">
							...
							<div class="col-xs-8 text-left">
								<span>同步到</span>
								<h2 class="font-bold">{{ $data['last_sync'] }}</h2>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<div class="col-xs-4">
							...
							{{--<span>平均在线 ACU  {{ $data['acu'] }}比 最高在线 PCU {{ $data['pcu'] }} </span>--}}
						</div>
						<div class="col-xs-8 text-left">
							<div>
						...
						<div class="tabs-container">
							<ul class="nav nav-tabs">
								<li class="active"><a data-toggle="tab" data-method="new"
								                      data-filled="true" href="#tab-1" data-download-area="#download-key" data-download-link="?export=true&fetch=login-register&from={{ $data['display_from'] }}&to={{ $data['display_to'] }}">新增玩家</a>
								</li>
								<li class=""><a data-toggle="tab" data-method="active"
								                href="#tab-2" data-download-area="#download-key" data-download-link="/home/json?method=active&export=true&fetch=active&from={{ $data['display_from'] }}&to={{ $data['display_to'] }}">活跃玩家</a>
								</li>
								<li class=""><a data-toggle="tab" data-method="payed"
								                href="#tab-3" data-download-area="#download-key" data-download-link="/home/json?method=payed&export=true&fetch=payed&from={{ $data['display_from'] }}&to={{ $data['display_to'] }}">付费玩家</a>
								</li>
								<li class=""><a data-toggle="tab" data-method="incoming"
								                href="#tab-4" data-download-area="#download-key" data-download-link="/home/json?method=incoming&export=true&fetch=incoming&from={{ $data['display_from'] }}&to={{ $data['display_to'] }}">收入</a>
								</li>
							</ul>
				...
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<div class="tabs-container">
							<ul class="nav nav-tabs">
								<li class="active">
									<a data-toggle="tab" href="#tab-pay-1" data-filled="true" data-method="pur"  data-download-area="#download-pay" data-download-link="?export=true&fetch=pay-percent&from={{ $data['display_from'] }}&to={{ $data['display_to'] }}&by=day">付费率</a>
								</li>
								<li >
									<a data-toggle="tab" href="#tab-pay-2" data-method="arpu"  data-download-area="#download-pay" data-download-link="/home/json?method=arpu&export=true&fetch=arpu&from={{ $data['display_from'] }}&to={{ $data['display_to'] }}&by=day">ARPU</a>
								</li>
								<li class="">
									<a data-toggle="tab" href="#tab-pay-3" data-method="arppu"  data-download-area="#download-pay" data-download-link="/home/json?method=arppu&export=true&fetch=arppu&from={{ $data['display_from'] }}&to={{ $data['display_to'] }}&by=day">ARPPU</a>
								</li>
								<li class="">
									<a data-toggle="tab" href="#tab-pay-4" data-method="au-avg"  data-download-area="#download-pay" data-download-link="/home/json?method=au-avg&export=true&fetch=au-avg&from={{ $data['display_from'] }}&to={{ $data['display_to'] }}&by=day">活跃用户平均付费</a>
								</li>
							</ul>
							<div class="tab-content">
...
@section('script')
	...
	<script>
		$(document).ready(function () {
			...
			var lineData = {
				labels: [
					<?php $acuCount = count($data['acu_by_day']); $i = 0 ?>
							@foreach($data['acu_by_day'] as $key => $acu)
				...
				datasets: [
					{
						...
						pointHighlightStroke: "rgba(220,220,220,1)",
						data: [
							<?php $acuCount = count($data['pcu_by_day']); $i = 0 ?>
									@foreach($data['pcu_by_day'] as $key => $acu)
			...
			var lineData = {
				labels: [
					<?php $regCount = count($data['login_by_day']); $i = 0 ?>
							@foreach($data['login_by_day'] as $key => $acu)
							"{{ $acu['day'] }}" @if(++$i != $acuCount), @endif
					@endforeach
				],
				datasets: [
					{
						...
						pointHighlightStroke: "rgba(220,220,220,1)",
						data: [
							<?php $acuCount = count($data['login_by_day']); $i = 0 ?>
									@foreach($data['login_by_day'] as $key => $acu)
			...
			var lineData = {
				labels: [
					<?php $regCount = count($data['pur']); $i = 0 ?>
							@foreach($data['pur'] as $key => $acu)
							"{{ $acu['day'] }}" @if(++$i != $acuCount), @endif
					@endforeach
				],
				datasets: [
					{
						...
						pointHighlightStroke: "rgba(220,220,220,1)",
						data: [
							<?php $acuCount = count($data['pur']); $i = 0 ?>
									@foreach($data['pur'] as $key => $acu)
