			...
			<div class="col-lg-12 animated fadeInRight">
				<div class="ibox float-e-margins">
					...
					<div class="row">
						...
						?>
						<form action="{{ url('channel/roles') }}" method="GET"
						      id="search-form">
						...
						<div class="table-responsive">
							<table class="table table-striped">
								...
								@foreach($roles as $role)
									<tr>
										<td><a
													href="{{ url('channel/roles/' . $role->UserID) }}">{{ $role->UserID }}</a>
										</td>
										<td><a
													href="{{ url('channel/roles/' . $role->UserID) }}">{{ $role->RoleName }}</a>
										</td>
										<td>
											@if($role->Gender == 1)
												男
										...
										<td>{{ $role->Level }}</td>
										<td><a href="{{ url('channel/roles/' . $role->UserID) }}">物品和装备</a></td>
										<td>{{ $role->Gold }}</td>
										<td>
